...
qq --cwd-context "explain the files in this directory"
...
Built-in backends run from a fresh, empty temporary working directory on each invocation by default. Pass `--cwd-context` to run them from your current working directory instead.
...
| `--cwd-context` | Use the current working directory as backend context instead of the default empty temporary directory. |
...
- Built-in backends (`codex`, `claude`, and `gemini`) start in a fresh empty temporary directory by default, even if you override their `path` or `args` in config. Pass `--cwd-context` when you want the built-in backend to see your current directory instead.
